...
(function () {
  ...
  const PROVIDERS = {
    gemini: {
      ...
      placeholder: "AIza…",
      hint: 'Get a <strong>free</strong> key at <a href="https://aistudio.google.com/app/apikey" target="_blank" rel="noopener">aistudio.google.com</a> — free tier, no billing required.',
      models: [["gemini-2.0-flash", "Gemini 2.0 Flash (free, fast)"], ["gemini-2.5-flash", "Gemini 2.5 Flash (free)"], ["gemini-1.5-flash", "Gemini 1.5 Flash (free)"]]
    },
    claude: {
      label: "Claude",
      keyLabel: "Anthropic API key",
      placeholder: "sk-ant-…",
      hint: 'Get a key at <a href="https://console.anthropic.com/settings/keys" target="_blank" rel="noopener">console.anthropic.com</a> — uses paid API credits (a claude.ai subscription does not work here).',
      models: [["claude-opus-4-8", "Claude Opus 4.8 (most capable)"], ["claude-sonnet-5", "Claude Sonnet 5"], ["claude-haiku-4-5", "Claude Haiku 4.5 (cheapest)"]]
    }
  };
  const keyStore = (p) => "hrwr_key_" + p;
  ...
  function applyProvider(name) {
    ...
    $("model").innerHTML = p.models.map(([v, l], i) => `<option value="${v}"${i === 0 ? " selected" : ""}>${l}</option>`).join("");
    const saved = localStorage.getItem(keyStore(name));
    $("apiKey").value = saved || "";
    $("rememberKey").checked = !!saved;
  }
  function syncKey() {
    const name = $("provider").value;
    if ($("rememberKey").checked && $("apiKey").value.trim()) localStorage.setItem(keyStore(name), $("apiKey").value.trim());
  ...
  // ---- AI integration (bring-your-own-key: Gemini free tier or Claude) -
  $("aiBtn").addEventListener("click", generateWithAI);
  async function generateWithAI() {
    const provider = $("provider").value;
    const pLabel = (PROVIDERS[provider] || {}).label || "AI";
    const key = $("apiKey").value.trim();
    if (!key) { notice("aiNotice", `No API key set. Open <strong>AI analysis settings</strong> above and paste your ${escapeHtml(pLabel)} key, or keep using the built-in analysis.`, "err"); $("settingsBox").open = true; return; }
    if (!state.agg) return;
    const btn = $("aiBtn"); btn.disabled = true; const original = btn.innerHTML;
    btn.innerHTML = `<span class="spinner"></span> Asking ${escapeHtml(pLabel)}…`; notice("aiNotice", "");
    try {
      const model = $("model").value;
      const prompt = buildPrompt(state.agg);
      const data = provider === "gemini" ? await callGemini(key, model, prompt) : await callClaude(key, model, prompt);
      renderAnalysis(data, `Written by ${pLabel} (${model}) · for HR stakeholders.`);
      notice("aiNotice", `Analysis generated by ${escapeHtml(pLabel)}.`, "ok");
    } catch (err) {
      notice("aiNotice", `${escapeHtml(pLabel)} request failed: ` + escapeHtml(err.message) + " — showing built-in analysis instead.", "err");
      renderAnalysis(builtInAnalysis(state.agg), "Built-in analysis (AI call failed).");
  ...
  function buildPrompt(agg) {
    ...
    return `You are an HR operations analyst writing a weekly status report for HR stakeholders (non-technical managers).
...
${JSON.stringify(ctx, null, 2)}`;
  ...
  async function callGemini(key, model, prompt) {
    ...
    if (!res.ok) {
      let detail = res.status + " " + res.statusText;
      try { const j = await res.json(); if (j.error && j.error.message) detail = j.error.message; } catch (_) {}
      if (res.status === 400 && /API key/i.test(detail)) detail = "Invalid API key.";
      if (res.status === 429) detail = "Free-tier rate limit hit — wait a moment and retry.";
      throw new Error(detail);
    }
    const json = await res.json();
    const cand = (json.candidates || [])[0];
    const text = cand && cand.content && cand.content.parts ? cand.content.parts.map((p) => p.text || "").join("\n") : "";
    if (!text) throw new Error("Empty response" + (cand && cand.finishReason ? " (" + cand.finishReason + ")" : "") + ".");
    return parseModelJson(text);
  }
